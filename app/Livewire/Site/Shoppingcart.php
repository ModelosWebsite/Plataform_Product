...
class Shoppingcart extends Component
{
    ...
    public $number = 0, $localizacao = 0, $cartContent, $getTotal, $getSubTotal, $getTotalQuantity, $cupon, $taxapb = 0,
    $finalCompra, $totalFinal = 0, $code, $delveryId, $receipt, $deliveryType, $qtd = [];

    // Dados de checkout
    public $name, $lastname, $province, $municipality, $street, $phone, $otherPhone,
    $email, $deliveryPrice = 0, $taxPayer, $otherAddress, $deliveryUrl, $latitude, $longitude,
    $referenceNumber, $paymentType, $bankAccount, $package, $cart = 0;
    ...
    {
        return pacote::where('company_id', $this->companyId)->where('is_active', true)->where('package_name', 'Transferência')->latest()->first();
    }
    ...
    {
        try {
            $response = Http::withHeaders($this->getHeaders())->post(env('LINK_KITUTES') . '/cupons', [
                'code' => $this->code,
